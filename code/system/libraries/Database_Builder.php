...
class Database_Builder_Core {
	...
	/**
	 * Добавление юниксового поля даты
	 * 
	 * @created by Antuan
	 */
	public function date($field_name){
		if (!is_object($this->db)) {
			$table_prefix = Kohana::config('database.'.$this->db.'.table_prefix');
		} else {
			$table_prefix = $this->db->table_prefix();
		}
		
		if(strpos($field_name, '.')){
			list($pefix, $field_name) = explode('.', $field_name);
			$field_name_full = $table_prefix . $pefix . '.' . $field_name;
			
		} else {
			$field_name_full = $field_name;
		}
		
		$this->select(new Database_Expression("UNIX_TIMESTAMP({$field_name_full}) as `{$field_name}_u`"));
	...
	public function restore_settings()
	{
		...
		$query_history = array_pop($this->query_history);
		array_push($this->query_history, $query_history);
