 ...
 * GD Image Driver.
 *
 * $Id: GD.php 4551 2009-09-04 22:52:22Z isaiah $
 *
 * @package    Image
 * @author     Kohana Team
...
class Image_GD_Driver extends Image_Driver {
	...
	public function resize($properties)
	{
		...
		/**
		 * Хак для Нересайза маленьких изображений
		 * 
		 * @modified by Antuan
		 */
		if (!empty($properties['width']) AND !empty($properties['height']) AND $properties['master'] === Image::AUTO_NOSTRETCH)
		{
			...
			$tmpWidth=$width;
             ...
             if($properties['width']<$tmpWidth){
                     $tmpWidth=$properties['width'];
                     $tmpHeight=$properties['width']/$fltScale;
                     $changed=true;
             }

             if($properties['height']<$tmpHeight){
                     $tmpHeight=$properties['height'];
                     $tmpWidth=$properties['height']*$fltScale;
                     $changed=true;
             }
             ...
             if(!$changed){
                  ...
                  $properties['width']=$tmpWidth;
                  $properties['height']=$tmpHeight;
             }
             
             $properties['width']=intval($properties['width']);
             $properties['height']=intval($properties['height']);
		}
