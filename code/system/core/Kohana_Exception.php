 ...
 * Kohana Exceptions
 *
 * $Id: Kohana_Exception.php 4669 2009-11-06 05:48:22Z isaiah $
 *
 * @package    Core
 * @author     Kohana Team
...
class Kohana_Exception_Core extends Exception {
	...
	public static function handle(Exception $e)
	{
		try
		{
			 ...
			 */
			$template_mail = FALSE;
			if(Kohana::config('core.sending_errors.enable') === TRUE) {
				if(Kohana::config('core.sending_errors.type') == 'detail')
					$template_mail = '_detailmail';
				else
					$template_mail = '_mail';
			}
			...
			if ($e instanceof Kohana_Exception)
			{
				 ...
				 */
				if ($template_mail)
					$template_mail = $e->getTemplate().$template_mail;
			...
			else
			{
				 ...
				 */
				if ($template_mail)
					$template_mail = Kohana_Exception::$template.$template_mail;
			...
			if ($template = Kohana::find_file('views', $template))
			{
				 ...
				 */
				if($template_mail) {
					if ($template_mail = Kohana::find_file('views', $template_mail)) {
					    ob_start();
					    include $template_mail;
					    $content = ob_get_contents();
					    ob_end_clean();
						
						Kohana_Exception::_send($content);						
					}
				}
			}
		}
		catch (Exception $e)
		{
			 ...
			 */
			if(Kohana::config('core.sending_errors.enable') === TRUE) {
				Kohana_Exception::_send(Kohana_Exception::text($e));
			}
		}
	...
	private static function _send($message){
		...
		email::send(Kohana::config('core.sending_errors.email'), "kohana@{$host}", "error on {$host}", $message, TRUE);
	}
