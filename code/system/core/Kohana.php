 ...
 *
 * $Id: Kohana.php 4678 2009-11-09 16:58:33Z kiall $
 *
 * @package    Core
 * @author     Kohana Team
...
abstract class Kohana_Core {
	...
	public static function & instance()
	{
		if (Kohana::$instance === NULL)
		{
			...
			if(!empty(Router::$controller_path)) {
			 ...
			 * @created by Antuan
			 */
			$class->getMethod('_before')->invoke($controller);
			 ...
			 * @created by Antuan
			 */
			$class->getMethod('_after')->invoke($controller);
	...
	/**
	 * Set config item or group proxies Kohana_Config.
	 *
	 * @param   string   item name
	 * @param   boolean  force a forward slash (/) at the end of the item
	 * @param   boolean  is the item required?
	...
	{
		return Kohana_Config::instance()->set($key,$value);
	}
		...
		if ($lifetime > 0)
		{
			...
			if (is_file($path))
			{
				...
				if ((time() - filemtime($path)) < $lifetime)
				{
					...
					{
						//modifyed by Sinner, go out fucking error
						$intFilesize = filesize($path);
						$strCacheContent = file_get_contents($path);

						if($intFilesize <> strlen($strCacheContent) || (time() - filemtime($path)) >= $lifetime-60){
							//unlink($path);
							return NULL;
						}

						return unserialize($strCacheContent);
					}
				}
				else
				{
