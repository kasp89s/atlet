...
<div id="kohana-debug-toolbar">
	...
	<div id="debug-toolbar" class="debug-toolbar-align-<?php echo $align ?>">
		...
		<?php if (Kohana::config('debug_toolbar.minimized') === TRUE): ?>
			<ul id="debug-toolbar-menu" class="menudebug" style="display: none">
		<?php else: ?>
			<ul id="debug-toolbar-menu" class="menudebug">
		<?php endif ?>
	...
	<?php
		...
		foreach ((array)$queries as $id => $query){
			foreach ((array)$queries as $id2 => $query2){
				if($query['query'] == $query2['query'] && $id != $id2)
					$queries[$id]['alert'] = true;
			}
		}
	?>
		<div id="debug-database" class="top" style="display: none;">
			...
			<table cellspacing="0" cellpadding="0">
				...
				<?php foreach ((array)$queries as $id => $query): ?>
					<tr class="<?php echo text::alternate('odd','even')?>" onclick="debugToolbar.toggle('vars-queries1-<?php echo $id ?>'); debugToolbar.toggle('vars-queries2-<?php echo $id ?>'); return false;">
						<td><?php echo $id + 1 ?></td>
						<td style="cursor: pointer;">
							<div id="vars-queries1-<?php echo $id ?>">
								<?php
									if(!empty($query['alert'])) echo html::image(Kohana::config('debug_toolbar.icon_path').'/alert.png', array('alt' => 'memory', 'width' => '16'));
									echo $query['query'];
								?>
							</div>
							<div style="display: none;" id="vars-queries2-<?php echo $id ?>">
								<?php
									if(!empty($query['alert'])) echo html::image(Kohana::config('debug_toolbar.icon_path').'/alert.png', array('alt' => 'memory', 'width' => '16'));
