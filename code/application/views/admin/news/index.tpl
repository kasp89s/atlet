...
<p>
	<label for="cat">Категория новостей</label>
...
<form action="/admin/news/group" method="POST">
<table cellspacing="0" cellpadding="0" class="table">
	<thead align="left" valign="middle">
        ...
        <td nowrap>Превью</td>
        {if 'news_publication'|acl_is_allowed}<td width="1%" title="Активность">Акт.</td>{/if}
	...
	<tbody align="left" valign="middle">
	    ...
	    <tr class="row_p">
			...
			{/if}
			{if 'news_del'|acl_is_allowed}<td align="center"><a href='/admin/news/delete?id={$item.id}' class='confirm'><img src='/i/admin/delete.png' alt='Delete' title='Delete'></a></td>{/if}
			{if 'news_edit'|acl_is_allowed}<td><a href="/admin/news/edit?id={$item.id}">Ред.</a></td>{/if}
	    </tr>
	    {/foreach}
	</tbody>
</table>
