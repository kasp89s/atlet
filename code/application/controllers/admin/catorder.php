 ...
 *
 */
class Catorder_Controller extends Admin_Controller {
	...
	public function index() {
		...
		$this->template = new View('admin/catorder/index');

		$table = new Catorder_Model;
		$table->db
	...
	public function edit() {
		$this->template = new View('admin/catorder/edit');
		...
		if($id){
			...
			if(!$data) message::error('Некорректный идентификатор (id) статуса', '/admin/catorder');
		} else
			$data = array();
			...
			if($form->is_ok()){
				...
				if($id) {
					if(!$result = $table->update($fill, array('id'=>$id)))
						...
						log::add('order', 'Редактирование статуса id='.$id);
					...
					if(!$result = $table->insert($fill)) {
						...
						$id = $result->insert_id();
						log::add('order', 'Добавление статуса id='.$id);
					}
				}
			}

			if($form->is_ok()){
				message::info('Статус успешно сохранен', '/admin/catorder');
			} else {
	...
	public function group() {
		if(!Acl::instance()->is_allowed('order_edit'))
			message::error('Нет прав доступа для сохранения', '/admin/catorder');
		...
		sort($ids);
		log::add('order', 'Групповое сохранение статусов id=[' . implode(',', $ids) . ']');

		message::info('Данные успешно сохранены', '/admin/catorder');
	}
	...
	public function delete() {
		if(!Acl::instance()->is_allowed('order_del'))
			message::error('Нет прав доступа к данному разделу', '/admin');
		...
		log::add('order', 'Удаление статуса id='.$id);

		message::info('Статус успешно удален', '/admin/catorder');
	}
}
?>