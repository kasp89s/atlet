...
function loadResults(data,voting_id,archives_uri,isShowArchivesLink) {
	var total_votes = 0;
	var percent;
	...
	var strQuestionText=$('#pollQuestion' + voting_id).text();

	var results_html = "<div class='poll_results_container_right' id='poll-results"+voting_id+"'><b>Результаты голосования:</b>\n<div class='graph'>\n";
	var answersLegend="";
	var counter=0;
	for (id in data) {
	    counter++;
    ...
    results_html = "<table class='votingsTable'><tr><td class='firstCol'><h3>"+strQuestionText+"</h3><br clear='all'>"+answersLegend + (isShowArchivesLink?"<br /><br /><a href='"+archives_uri+"'>Архив опросов</a>":"")+"</td><td class='secondCol'>" + results_html + "</td></tr></table>";

	$("#poll-container"+voting_id).empty();
	$('#pollQuestion' + voting_id).empty();
